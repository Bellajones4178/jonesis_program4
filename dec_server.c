...
int main(int argc, char *argv[]) {
    ...
    while (1) {
        ...
        if (pid == 0) {
            ...
            char *cipher = malloc(msgLen + 1);
            char *key = malloc(keyLen + 1);
            if (!cipher || !key) { fprintf(stderr, "ERROR: out of memory\n"); close(connectSock); exit(1); }

            recvAll(connectSock, cipher, msgLen);
            recvAll(connectSock, key, keyLen);
            cipher[msgLen] = '\0';
            key[keyLen] = '\0';

            char *plain = malloc(msgLen + 1);
            if (!plain) { fprintf(stderr, "ERROR: out of memory\n"); free(cipher); free(key); close(connectSock); exit(1); }

            // decrypt
            decrypt(cipher, key, plain);

            // send back
            sendAll(connectSock, plain, msgLen);
